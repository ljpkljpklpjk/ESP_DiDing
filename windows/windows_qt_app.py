...
class TitratorQtApp(QMainWindow):
    ...
    def __init__(
        ...
        self.command_id = 1
        self.system = WindowsSystemManager(project_dir)
        ...
        self.setWindowTitle("ESP32 自动滴定仪上位机 (Windows)")
    ...
    def refresh_wifi_status(self):
        self.run_system_task(
            "刷新 WiFi 状态", self.system.wifi_status, self.network_page.set_status
        )

    def wifi_on(self):
        self.run_system_task(
            "打开 WiFi", self.system.wifi_on, self.network_page.set_status
        )

    def wifi_off(self):
        self.run_system_task(
            "关闭 WiFi", self.system.wifi_off, self.network_page.set_status
        )

    def connect_wifi(self, ssid, password):
        self.run_system_task(
            "连接 WiFi",
            lambda: self.system.connect_wifi(ssid.strip(), password),
            self.network_page.set_status,
        )

    def check_project_update(self, project_dir):
        self.system.project_dir = Path(project_dir).expanduser().resolve()
        self.run_system_task(
            "检查项目更新", self.system.check_git_update, self.update_page.set_status
        )

    def update_project(self, project_dir):
        self.system.project_dir = Path(project_dir).expanduser().resolve()
        self.run_system_task(
            "更新上位机代码", self.system.update_project, self.update_page.set_status
        )
    ...
    def update_esp32_ota(self, project_dir, host, password):
        ...
        if not self.system.firmware_path().exists():
            self.update_page.set_status(
                f"ESP32 OTA 更新失败: 未找到预编译固件 {self.system.firmware_path()}"
            )
            return
        ...
        append_log(
            self.update_page.log, "执行命令: " + " ".join(cmd), max_lines=800
        )
    ...
    def update_ota_heartbeat(self):
        ...
        self.update_page.set_progress(
            f"OTA 运行中，已用 {elapsed}s，最近输出 {idle}s 前"
        )
    ...
    def finish_ota(self, code, last_line):
        ...
        if code == 0:
            ...
            append_log(
                self.update_page.log,
                f"OTA 任务失败: {last_line or '无输出'}",
                max_lines=800,
            )
    ...
    def handle_message(self, msg):
        ...
        if msg_type == "boot":
            ...
            self.status_label.setText(
                f"命令 {msg.get('id', '--')} 完成 ok={msg.get('ok')}"
            )
        elif msg_type == "error":
            self.status_label.setText(
                f"错误: {msg.get('code', msg.get('message', '--'))}"
            )
