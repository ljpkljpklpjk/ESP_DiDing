...
- 版本号：v2026.05.30.2
- 提交时间：2026-05-30 22:26:57 +0800
- 更新内容：修复 ADS1220 寄存器读写命令格式，启用 pH/TDS 差分输入所需的 PGA bypass，DRDY 使用内部上拉，并把转换等待超时调整为 100ms；重新生成预编译 OTA 固件，恢复 pH/TDS 电压读取稳定性。
...
ADS1220 驱动默认配置为 PGA bypass、90 SPS 单次转换，DRDY 使用内部上拉，单次读取超时为 100ms。这样适配 AIN1/AGND 参考和低阻抗 pH/TDS 模块输出，避免寄存器配置失败或转换未完成时电压显示为空。
